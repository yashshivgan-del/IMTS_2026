...
def validate_placement() -> dict:
    ...
    prompt = (
        ...
        "Keep Steps 1-5 brief — a few short lines each, no markdown tables, no headers. "
        "Map each raw visual cell from Step 1 to its label, reusing the exact colors/empty status "
        "you recorded in Step 1 (do not re-guess colors here).\n\n"
        "FINAL OUTPUT FORMAT — this exact format is REQUIRED, do not use a markdown table, "
        "do not add extra headers or commentary around it, output exactly these 7 lines with "
        "no bold/markdown formatting on the labels:\n"
        "- A1: [color or empty]\n"
        "- A2: [color or empty]\n"
        "- B1: [color or empty]\n"
        "- B2: [color or empty]\n"
